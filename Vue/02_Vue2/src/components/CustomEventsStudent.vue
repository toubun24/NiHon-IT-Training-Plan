...
// const emitter = mitt(); // 创建 mitt 事件发射器实例
export default {
    name: "CustomEventsStudent",
    data() {
        console.log(this);
        return {
            name: '张三',
    ...
    methods: {
        ...
        sendStudentName() {
            ...
            // emitter.emit('personalEvent', this.name, 666, 777, 888); // mitt
        ...
        unbind() {
            ...
            // emitter.off('personalEvent') // mitt
            console.log(".$off()已弃用")
            //this.$off([ 'personalEvent', 'demo' ]);// 解绑多个事件，参数为包含多个事件名的数组
            // this.$off(); //比较暴力，有几个自定义事件就全给你解绑了
        },
        death() {
            // this.$destroy(); // 销毁当前组件实例，销毁后所有该实例的自定义事件都不奏效了 // Vue 2
            console.log(".$destroy()已弃用")
        }
    }
}
</script>
