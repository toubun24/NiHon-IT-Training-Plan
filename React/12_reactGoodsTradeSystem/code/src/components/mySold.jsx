...
const MySold = ({ stateInfo }) => {
  ...
  useEffect(async () => {
    const res = Number(stateInfo) >= 0 ? await axios.get(`http://localhost:5000/trades/?sellerId=${myContent}&_sort=editTime&_order=desc&state=${stateInfo}`) : // 按时间降序 // desc // state_ne
      await axios.get(`http://localhost:5000/trades/?sellerId=${myContent}&_sort=editTime&_order=desc`)
